...
| ip\_filter | The IP filter configuration for the bucket. | <pre>object({<br>    mode = string<br>    public_network_source = optional(object({<br>      allowed_ip_cidr_ranges = list(string)<br>    }))<br>    vpc_network_sources = optional(list(object({<br>      network                = string<br>      allowed_ip_cidr_ranges = list(string)<br>    })))<br>  })</pre> | `null` | no |
