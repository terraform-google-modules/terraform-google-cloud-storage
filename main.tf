...
locals {
  ...
  names        = toset([for name in var.names : lower(name)])
  bucket_names = zipmap(var.names, google_storage_bucket.buckets[*].name)
}
...
resource "google_storage_bucket" "buckets" {
  ...
  labels             = merge(var.labels, { name = "${local.prefix}${each.name}" })
  force_destroy      = lookup(var.force_destroy, each.name, false)
  bucket_policy_only = lookup(var.bucket_policy_only, each.name, true)
  versioning {
    enabled = lookup(var.versioning, each.name, false)
  }
}

resource "google_storage_bucket_iam_binding" "admins" {
  for_each = var.set_admin_roles ? local.names : {}
  bucket   = local.buckets[each.name]
  role     = "roles/storage.objectAdmin"
  members = compact(concat(
    var.admins,
    split(",", lookup(var.bucket_admins, each.name, ""))
  ))
}

resource "google_storage_bucket_iam_binding" "creators" {
  for_each = var.set_creator_roles ? local.names : {}
  bucket   = local.buckets[each.name]
...
resource "google_storage_bucket_iam_binding" "storage_admins" {
  ...
  members = compact(concat(
    var.storage_admins,
    split(",", lookup(var.bucket_storage_admins, each.name, ""))
  ))
}
